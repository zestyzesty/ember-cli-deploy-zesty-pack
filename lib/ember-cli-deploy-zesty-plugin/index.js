  ...
  createDeployPlugin: function(options) {
    return {
      ...
      didActivate: function(context) {
        ...
        if (context.deployTarget === 'production') {
          return this.notifySlackOfActivation(context, this.slackConfig);
        }
      },

      notifySlackOfActivation: function(context, slackConfig) {
        ...
        var username = 'Ember - ' + title + ' ' + deployTarget;
        var shortRev = context.revisionData.revisionKey.slice(0, 7);
        var message = "@" + slackConfig.githubUser + " deployed version " + shortRev + " of " + name + ".";

        var slack = new SlackApi(slackConfig.webhook, {
          channel: channel,
          username: username
        });
